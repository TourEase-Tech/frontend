...
const TourClientDetailPage = () => {
  ...
  useEffect(() => {
    const fetchRecommendedTours = async () => {
      try {
        const response = await fetch(`https://ai-c9ka.onrender.com/recommend?tour_id=${queryTourConfig.id}`)
        if (!response.ok) {
    ...
    <Fragment>
      ...
      {tour ? (
        <div className='flex flex-col mx-auto px-20 py-10 gap-4'>
          ...
          {likeOrUnlikeCommandHandler.isLoading() ? (
            ...
            <div className='mb-5 flex items-start gap-2'>
              <Button
                classNameButton=''
                onClick={() =>
                  handleLikeOrUnlikeTour({
                    tour: queryTourConfig.id as string,
                    action: tour.isLiked ? 'unlike' : 'like'
                  })
                }
              >
                {tour.isLiked ? (
          ...
          <div className='grid grid-cols-3 gap-4'>
            ...
            <div className='col-span-3 flex flex-col w-full'>
              <TourSlider heading='Tour Similiar' tours={recommendedTours} />
            </div>
          </div>
        </div>
      ) : (
